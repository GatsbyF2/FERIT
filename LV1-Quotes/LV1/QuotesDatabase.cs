 ...
 {"KB", new Author("Kent", "Beck", new DateOnly(1961, 3, 31)) },
    ...
    private IList<Quote> LoadQuotes()
    {
        return new List<Quote>()
        {
            ...
            "well over 10 to 1. We are constantly reading old code as part of the effort to write new code. Making it easy to read makes it easier to write."),
    ...
    public IList<Quote> GetQuotesByAuthor(string name, string lastName)
    {
        return quotes.Where(q => q.Author.Name.ToLower().Equals(name.ToLower())).ToList().AsReadOnly();
    }
}
